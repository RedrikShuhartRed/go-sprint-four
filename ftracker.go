...
func RunningSpentCalories(action int, weight, duration float64) float64 {
	...
	runSpeed := meanSpeed(action, duration)
	SpentCalories := (runningCaloriesMeanSpeedMultiplier * runSpeed * runningCaloriesMeanSpeedShift) * weight / mInKm * duration * minInH
...
func WalkingSpentCalories(action int, duration, weight, height float64) float64 {
	...
	walkingSpeedMsex := meanSpeed(action, duration) * kmhInMsec
	SpentCalories := (walkingCaloriesWeightMultiplier*weight + math.Pow(walkingSpeedMsex, 2)/(height/cmInM)*(walkingSpeedHeightMultiplier*weight)) * duration * minInH
	return SpentCalories
}
...
func SwimmingSpentCalories(lengthPool, countPool int, duration, weight float64) float64 {
	...
	SpentCalories := (swimSpeed + swimmingCaloriesMeanSpeedShift) * swimmingCaloriesWeightMultiplier * weight * duration
	return SpentCalories
